...
$(function(){       // window.onload - 브라우저 화면표시 완료
    ...
    if(storageData){        // localStorage에 저장된 값이 있는가?
        record = storageData;
        console.log( record );
    }else{
...
function userSelect(){
    ...
    // userImg클래스들을 배열로 가져오고 그 중에서 몇번째 인덱스 클릭했는가?
    $(this).css('border', '1px solid white');     // 내가 클릭한 가위바위보 이미지 표시
    clearInterval(comHd);       // 컴퓨터의 가위바위보 이미지 변경 하는거 멈추기 (clearInterval)
    ...
    // $(this).removeAttr('style');             // style 전체 날리기
    $(".result").remove();     
...
function comStart(){
    comHd = setInterval( function(){
        if( comSelect == 2 ) comSelect = -1;            // ++가 앞에 있는 조건식
    //    if( comSelect == 3 ) comSelect = 0;           // ++가 뒤에 있는 조건식
        $("#comImg").attr('src', './static/image/'+ com[++comSelect]);
    }, 100 );                // 지정된 시간에 한번씩 실행, 시간단위는 밀리세컨드 1000이 1초
